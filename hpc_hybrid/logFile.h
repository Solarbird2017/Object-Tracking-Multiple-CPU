...
void writeResultsToLog(string timecom, string Nsteps, string NPUs,
        string NPs, string rmse,  string NPstransGlo,
        string NPstransLoc, string TimeInterval, string outPutFileName ){
    string logFile = "log_" + outPutFileName + ".txt";
    fstream filestr;
    filestr.open (logFile.c_str(), fstream::in | fstream::out | fstream::trunc);

    filestr << "Results of Hybrid Resampling: " << endl;
